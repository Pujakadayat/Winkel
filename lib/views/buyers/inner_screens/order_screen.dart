...
//     return userSnapshot.exists ? userSnapshot.data() as Map<String, dynamic> : {};
//   }

//   Future<Map<String, dynamic>> fetchVendorDetails(String vendorId) async {
//     final vendorSnapshot = await FirebaseFirestore.instance.collection('vendors').doc(vendorId).get();
//     return vendorSnapshot.exists ? vendorSnapshot.data() as Map<String, dynamic> : {};
//   }
...
//                             ),
//                             title: Text(orderData['productName'] ?? 'Product Name Not Available'),
//                             subtitle: Column(
...
class CustomerOrderScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Stream<QuerySnapshot> _ordersStream = FirebaseFirestore.instance
        .collection('orders')
        .where('buyerId', isEqualTo: FirebaseAuth.instance.currentUser!.uid)
        .where('delivered', isEqualTo: false) // Exclude delivered orders
        .snapshots();
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            onPressed: () {
              // Navigate to Rating View Page
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => RatingViewPage()),
              );
            },
